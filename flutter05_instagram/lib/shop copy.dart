...
class _ShopState extends State<Shop> {
  ...
  getData() async {
    ...
    try {
      ...
      // 서울에 사는 사람만 가져오기
      var queryByCity = await firestore
          .collection('person')
      ...
      var queryByage = await firestore
          .collection('person')
      ...
      var queryByhobbies = await firestore
          .collection('person')
      ...
      // 여러 도시 조건
      var queryByCities = await firestore
          .collection('person')
      ...
      // 여러 이름 조건
      var queryByNames = await firestore
          .collection('person')
      ...
      var queryByAgeOrder = await firestore
          .collection('person')
          .where('age', isGreaterThanOrEqualTo: 30)
          .orderBy('age')
          .get();
      ...
      var queryByAgeOrderName = await firestore
          .collection('person')
          .where('age', isGreaterThanOrEqualTo: 30)
          .orderBy('name')
          .get();
      ...
      itemBuilder: (context, index) {
        ...
        return Row(
          children: [
            ...
            Expanded(child: Text(doc['age'].toString())),
            Expanded(child: Text(doc['city'])),
            Expanded(child: Text(doc['hobbies'].join(', '))),
          ],
        );
      },
    );
